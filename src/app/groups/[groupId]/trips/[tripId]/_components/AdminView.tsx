...
export function AdminView({ tripId }: { tripId: string }) {
    ...
    const handleProcessInvoice = async () => {
        ...
        const fileToScan = invoiceFile;

        setScanStep('processing');
        try {
            // 1. OCR
            const text = await scanInvoice(imageToFormData(fileToScan));
            if (!text) throw new Error('Falha no OCR');
