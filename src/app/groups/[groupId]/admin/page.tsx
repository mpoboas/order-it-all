...
import { tripsApi, subscriptions, groupsApi } from '@/lib/pocketbase';
...
export default function GroupAdminDashboardPage() {
    ...
    const loadTrips = useCallback(async () => {
        if (!groupId) return;
        try {
            // Admins see all trips (open and closed)
            const open = await tripsApi.getOpenByGroup(groupId);
            const closed = await tripsApi.getClosedByGroup(groupId);
            setTrips([...open, ...closed].sort((a, b) => new Date(b.created).getTime() - new Date(a.created).getTime()));
        } catch (error) {
                            ...
                            {trips.length === 0 ? (
                                ...
                                trips.map(trip => (
                                    <div
                                        ...
                                        className="card p-4 hover:shadow-md transition-shadow cursor-pointer relative group"
                                    >
                                        <div className="flex justify-between items-start mb-2">
                                            <h3 className="font-bold text-[var(--text-primary)] truncate pr-2">{trip.name}</h3>
